...
G4 ChangePlan 工具现可分别冻结 `deepseek_intent_policy` 与 `deepseek_explanation_policy`：Explanation 计划必须同时启用 Intent，绑定条目上限、每项最多两次尝试、四路并发、证据输入范围、逐项模板回退和重放零调用。执行器会从持久化 AgentResult 回读每条解释，检查 Agent 版本、provider、fallback、尝试总数、解释数量以及每个白名单引用的精确方括号标记；未获新 plan_id/hash 批准时不会执行业务 POST。
