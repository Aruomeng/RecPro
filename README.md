...
G8 发布候选前置检查已加入 `scripts/verify_g8_release_preflight.py` 与 `make verify-g8-release-preflight`：它会在新证据目录中运行契约、文档、架构、安全、G1/G4/G5/G7/G8 测试，生成新的前端构建，检查后端镜像和默认 fail-closed 配置，并写入源码 SHA-256 清单。首次实际报告 `artifacts/verification/g8/g8-release-preflight-20260812-001/release-preflight.json` 为 `PASS_WITH_BLOCKERS`：12 项技术检查全部通过，源码清单 402 个文件；该命令只读检查和构建本地产物，不启动应用、不连接数据库、不 claim Outbox、不调用 DeepSeek。A01—A25 最终复验、六场景浏览器 E2E、生产认证和真实 G5 写入仍保持阻塞。
