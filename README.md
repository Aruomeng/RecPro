...
G7 现在另有独立的 `backend.app.g4_feedback_demo_main:app` 入口：必须同时通过 G4/G5 后端双开关、经 `AppSettings` 校验的同名设置、隔离 Graph/Vector 运行时和 G5 服务注入，才会挂载真实 feedback/behavior POST；默认 Compose backend、Worker 和 DeepSeek 仍关闭。前端交互按钮默认关闭的浏览器验收已通过，390×844 无横向溢出，未产生交互 POST。
...
为避免把离线覆盖误当成最终验收，新增 `contracts/verification/g8-final-revalidation-plan.schema.json`、`scripts/build_g8_final_revalidation_plan.py` 和 `make build-g8-final-revalidation-plan`。最新只读计划 `artifacts/verification/g8/g8-final-revalidation-plan-20260812-002/final-revalidation-plan.json` 绑定提交 `dc178c15bbc5bcf56745aa1448c469ffff975462`、`plan_hash=c9ba9d61e264accc34605fe5a7dea22a982b8dfaa998040b5ec2565a57666ca2`，冻结 25 项 A01—A25 与六个浏览器场景；计划只允许读取/验证，业务 POST、Outbox claim、索引写入和 DeepSeek 请求均为 0，六个浏览器业务场景仍需各自精确 ChangePlan。
