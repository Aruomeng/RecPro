...
2026-08-29 的“证据约束、情境感知多智能体推荐”阶段已经完成：同步/异步个性化授权统一，客户端保留约束被拒绝；全局 Workspace 具备真实有界 Agent 调度和 SessionTopicGraph；知识图谱 v2、多跳路径证据、馆员审核闭环、全页面证据联动及开发性实验运行器均已实现。开发实验覆盖 B0—B3、Proposed、9 个消融和8类故障，严格标记为 `DEVELOPMENT_PROXY`。用户精确批准后，Workspace 审计追加17行、Neo4j v2追加78,129节点/206,848关系、知识审核追加266行，均已独立只读对账；v1 图计数零变化，DeepSeek请求和物理删除均为0。详见 [证据约束多智能体优化实施状态](docs/LibraMAS_证据约束多智能体优化实施状态_20260829.md)。
...
G8 的 A01—A25 核心运行态复验现已全部收口。当前提交绑定计划 `g8-final-revalidation-plan-20260814-003` 的 17 个只读/故障用例通过 64 项隔离断言；新增的只读对账器随后核验三份既有精确批准的 ChangePlan/apply 证据以及隔离 MySQL 中的任务、幂等 UUID、反馈、曝光边界、资源状态、负偏好、Outbox 和 change log 事实，将 A02/A03/A04/A07/A08/A09/A10/A23 提升为 PASS。最终审计 `g8-final-revalidation-audit-20260814-001` 为 `PASS`：`final_pass=25`、`final_pending=0`、`final_fail=0`。本轮对账仅执行 15 次 MySQL 读取并 rollback；数据库写入、Outbox claim、DeepSeek 请求、Neo4j/Chroma 写入、文件删除和数据库物理删除均为 0。六场景真实浏览器业务写入、生产 OIDC/JWKS 和 G9 正式评测输入仍是发布/论文实验层的独立工作，不影响上述 25 项核心语义已经通过的结论。

六场景浏览器工作的独立 DRY_RUN 计划生成器与验证器已完成，最新计划会以 `artifacts/verification/g8/g8-browser-scenario-plan-*/browser-scenario-plan.json` 追加保存，并绑定生成时的 clean Git commit。计划冻结 `demo_cold/demo_clear/demo_topic/demo_path/demo_negative/demo_degraded` 六个场景的 UUID、输入、期望状态、DOM 断言和 `deepseek-v4-flash` 调用上限；预计数据库最多追加 306 行、外部模型最多 80 次，Outbox、删除、Neo4j/Chroma 写入均为 0。每份计划均需单独核对其精确 plan_id/hash，批准前不会发送六场景业务 POST。
...
- [证据约束多智能体优化实施状态](docs/LibraMAS_证据约束多智能体优化实施状态_20260829.md)
