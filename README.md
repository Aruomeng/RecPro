...
DeepSeek 已通过本地“真实调用就绪预检”：`artifacts/verification/llm/llm-real-call-readiness-20260812-001/real-call-readiness.json`=`READY_FOR_EXPLICIT_OPT_IN`，provider/model/HTTPS/Prompt Bundle/key 存在性均通过。默认 Compose、HTTP、Worker 仍使用 Mock/关闭；真实调用只在显式研究组合根、固定非敏感 fixture、费用/超时/回退/审计边界和用户明确批准同时满足时发生。
...
随后新增 Intent-only 的 G4 只读组合与 `make verify-g4-real-llm-readonly`；离线 fake 回放通过，但获批的 G4 真实探针尚未形成 PASS artifact，不能声称真实推荐业务链路已经切换。修正后的探针需要新的 run id 和新的外部请求授权。
