...
完整 HTTP ChangePlan 通过 `G4_PROJECTION_ENABLE_DEEPSEEK_EXPLANATION=true` 独立纳入 Explanation；该开关要求 Intent 同时为 true。计划分别哈希 Intent/Explanation 的无密钥策略，按请求 `limit` 冻结 Explanation 条目数、最多 `limit × 2` 次尝试、四路并发、证据白名单和逐项回退。获批执行后必须回读持久化 ExplanationAgent 结果：无回退时验证 DeepSeek provider、版本、尝试边界、条目数及所有方括号证据标记；有回退时验证受控模板 provider、回退版本、条目数及尝试上限。幂等重放仍要求数据库零增量且不再次调用模型。对模型输出本身通过 JSON Schema、白名单与方括号标记的校验失败，现与 JSON 格式失败共用同一项最多两次的原始模型重试预算。
