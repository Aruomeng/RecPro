...
- `completed_work`：
  ...
  - 已完成 Agent 自主动作协议：在共享契约 `backend/app/shared_kernel/contracts/autonomy.py` 固化不可变角色目录，并由 `backend/app/recommendation/agents/autonomy.py` 提供兼容导出；8 个业务 Agent 固化 Role/Goal/Observation/Tools/AllowedActions/AllowedTargets；`AgentResult` 必须携带 `AgentDecision`，Registry 校验动作、目标和持久化 payload 镜像，越权或缺失动作直接阻断。
  - 已将 G4 Orchestrator 的澄清、降级、召回通道选择、排序结果和一次重规划分支改为读取 Agent 自主动作；新增 `scripts/verify_g4_agent_autonomy_runtime.py` 与 `make verify-g4-agent-autonomy`。四路径离线运行证据 `artifacts/verification/g4/g4-agent-autonomy-20260812-001/agent-autonomy-runtime.json` 以及共享契约归位和重规划动作复验 `...-002`、`...-003` 均=`PASS`，direct/guided/degraded/replanning 均通过，`REQUEST_REPLAN` 恰为 1 次，外部 LLM/数据库/图/向量/Outbox/删除/覆盖均为 0。
  - 已将动作协议接入显式 G4 HTTP/G5 反馈边界：`build_http_execution_payload` 对每个 trace action 做角色/动作校验并输出 `agent_actions`；G5 `FeedbackLearningAgent` 根据 receipt 的 pending/replayed/state 选择有限 `PROPOSE_PROFILE_DELTA`、`RETURN_RESULT` 或 no-op。新增 `scripts/verify_g4_http_feedback_autonomy.py` 与 `make verify-g4-http-feedback-autonomy`；最新离线证据 `artifacts/verification/g4/g4-http-feedback-autonomy-20260812-003/http-feedback-autonomy.json`=`PASS`，所有数据库/图/向量/Outbox/LLM/删除/覆盖计数均为 0。
  - 已新增 capability-specific LLM 组合参数 `llm_intent_provider`/`llm_explanation_provider`，并新增 `scripts/verify_g4_real_llm_readonly.py`；离线 fake provider 回放确认一次真实 Intent 能力不会触发 Explanation 的批量外部请求。真实运行 `g4-real-llm-readonly-20260812-002` 已使用 `deepseek-v4-flash` 完成一次 `intent.classify`，无 fallback，7 个 Agent 返回 8 个 `MYSQL+GRAPH+VECTOR` 候选，MySQL/Neo4j/Chroma 写入均为 0。
  - 已完成 DeepSeek Intent 的 HTTP 组合接线：新增 `RECPRO_G4_LLM_INTENT_ENABLED` 能力闸门；只有 demo + G4 HTTP + DeepSeek + Intent 四项同时成立，两个 G4 入口才把 provider 注入 Intent Agent。Explanation 保留证据模板，默认模块级 FastAPI、Compose 命令与 Worker 不自动启用；`make run-g4-deepseek-demo` 可启动已审查的本机入口。
  - 已把真实 DeepSeek HTTP 持久化纳入 G4 精确 ChangePlan：计划绑定 secret-free provider/model/Prompt/超时/尝试次数策略；执行器通过实际 FastAPI POST、同 request_id 重放和 GET 回读，要求第二次零行增量且只在首次请求调用 DeepSeek，并回读持久化 Intent Agent 的版本/provider/prompt/fallback/attempts。计划生成不写数据库、不调用模型，执行仍需用户批准新的精确 plan_id/plan_hash。
...
- `open_issues`：
  ...
  - LLM 配置、provider-only fixture、真实七 Agent 只读编排与 HTTP Intent 接线均已通过；剩余阻塞是生成一次精确 ChangePlan 后，通过实际 HTTP 推荐 POST 验证“DeepSeek Intent + G4 持久化 + 前端回读”的完整链路。默认模块级 FastAPI、Compose 命令与 Worker 继续 fail-closed。
...
| G4 动态多智能体闭环 | IN_PROGRESS | `artifacts/verification/g4/g4-orchestrator-20260809-001/orchestrator.json`；`artifacts/verification/g4/g4-agent-runtime-20260809-002/agent-runtime.json`；`artifacts/verification/g4/g4-real-ports-20260809-001/real-ports-runtime.json`；`artifacts/verification/g4/g4-composition-20260809-001/composition-runtime.json`；`artifacts/verification/g4/g4-agent-autonomy-20260812-001/agent-autonomy-runtime.json`；`artifacts/verification/g4/g4-agent-autonomy-20260812-003/agent-autonomy-runtime.json`；`artifacts/verification/g4/g4-http-feedback-autonomy-20260812-003/http-feedback-autonomy.json`；105 项 G4 测试 | Registry、结构化消息、角色/动作白名单、四路径、真实 Catalog/Profile 只读端口、bounded retry、显式组合根和同事务持久化 PASS；Policy/Ranking 自主动作驱动澄清与一次重规划；G4 HTTP trace action 投影和 G5 FeedbackLearningAgent 离线边界 PASS；Intent-only provider 组合不会触发 Explanation 批量请求；G4 真实 LLM 只读探针尚未形成 PASS artifact；正式 HTTP/Worker 接入、恢复读取和历史画像重算待完成 |
...
| G8 可靠性与发布候选 | IN_PROGRESS | `artifacts/verification/g8/g8-final-revalidation-audit-20260814-001/final-revalidation-audit.json`=`PASS`；A01—A25 为 25 PASS/0 PENDING/0 FAIL；只读故障矩阵 64 项断言 PASS；八项历史授权写入证据完成当前数据库对账 | 核心验收语义已全部通过；Gate 尚未标记 COMPLETED，因为六场景真实浏览器业务验收、生产认证/凭据、性能与独立发布环境复现仍属于发布候选退出条件 |
| G9 冻结实验 | IN_PROGRESS | `artifacts/verification/experiment/freeze-20260814-001/freeze-preflight.json` 与 `artifacts/verification/experiment-inputs/eval-inputs-20260814-001/input-freeze-report.json` 均为 `PASS_WITH_BLOCKERS` | F0 协议已冻结；仍缺确认性评测数据/许可 Manifest、盲标注、Split 和 F3 配置。现有 synthetic fixture 与 Lib 书目不能冒充用户相关性金标准 |
...
- `current_subtask`：G8 A01—A25 final revalidation 已达到 `25 PASS / 0 PENDING / 0 FAIL`。三批精确批准的写入证据已通过当前 MySQL 只读对账，统一审计为 PASS。下一步聚焦 G7 六场景真实浏览器业务验收、生产认证/发布复现，以及需要用户另行提供论文评测材料的 G9 F1—F3；默认 HTTP/Worker 仍保持 fail-closed，显式 G4 组合根已真实使用 DeepSeek Intent + Explanation 完成无回退任务。
...
```text
交接ID：LLM-REAL-CALL-READINESS-20260812-001
Gate：DeepSeek 外部调用前置检查
状态：READY_FOR_EXPLICIT_OPT_IN / NO_NETWORK_REQUEST
时间：2026-08-12（Asia/Shanghai）
目标：验证本机被忽略的 `.env.host` 是否具备一次显式 DeepSeek 调用所需的本地条件；只构造 provider 和校验 Prompt Bundle，不发送网络请求。
执行命令：`make PYTHON=.venv-g1-final-py311/bin/python LLM_REAL_CALL_ENV_FILE=.env.host LLM_REAL_CALL_READINESS_RUN_ID=llm-real-call-readiness-20260812-001 verify-llm-real-call-readiness`
实际结果：provider=`deepseek`，model=`deepseek-v4-flash`，HTTPS origin 有效，Prompt Bundle=`prompt-v1`、4 个任务、SHA-256 匹配；`external_call_authorized=false`、`network_probe_performed=false`。
验证证据：`artifacts/verification/llm/llm-real-call-readiness-20260812-001/real-call-readiness.json`。
安全计数：network_requests=0、external_llm_requests=0、database_reads=0、database_writes=0、outbox_claims=0、files_deleted=0、artifact_overwrites=0。
...
## G4 Agent 自主动作与动态调度记录

```text
交接ID：G4-AUTONOMY-20260812-001
Gate：G4 动态多智能体闭环（角色/动作协议与动态路径）
状态：CONTRACT_PASS / RUNTIME_PASS / G4_CONTINUES
时间：2026-08-12（Asia/Shanghai）
目标：将初始实施文档中的 Role/Goal/Observation/Tools/Policy/Action/Confidence/Trace 变成运行时可校验契约，避免把固定函数流水线或简单对话模型误称为自主 Agent。
新增文件：`backend/app/shared_kernel/contracts/autonomy.py`；兼容导出 `backend/app/recommendation/agents/autonomy.py`；`scripts/verify_g4_agent_autonomy_runtime.py`；`tests/g4/test_agent_autonomy.py`；`make verify-g4-agent-autonomy`。
修改文件：`backend/app/shared_kernel/contracts/enums.py` 增加有限 `AgentActionType`；`backend/app/shared_kernel/contracts/agent.py` 增加 `AgentDecision`；三个 Agent 实现辅助器写入并校验动作镜像；`AgentRegistry` 拒绝缺失/越权动作；`RecommendationOrchestrator` 将 Policy/Ranking 的动作作为澄清、降级和一次重规划的决策输入；trace 和 public Agent result 同时保留 action/target/reason/confidence。
角色边界：8 个业务 Agent 各有独立目标、观察字段、工具集合、允许动作和目标 Agent；Agent 不能直接推进全局状态或调用其他 Agent，Orchestrator 仍是唯一状态机所有者。
动态路径：direct=`PLAN_RECALL→SELECT_CHANNELS→RETURN_RESULT→RENDER_EVIDENCE`；guided=`ASK_CLARIFICATION` 后停在 `WAITING_CLARIFICATION`；degraded 记录 `DEGRADE`；replanning 由 RankingAgent 发出一次 `REQUEST_REPLAN`，重规划预算严格为 1。
验证命令：`PYTHONDONTWRITEBYTECODE=1 .venv-g1-final-py311/bin/python -m scripts.verify_g4_agent_autonomy_runtime --run-id g4-agent-autonomy-20260812-001`；共享契约归位后的复验命令为 `... --run-id g4-agent-autonomy-20260812-002`；G4 测试与 contracts 测试。
验证证据：`artifacts/verification/g4/g4-agent-autonomy-20260812-001/agent-autonomy-runtime.json`、`.../g4-agent-autonomy-20260812-002/agent-autonomy-runtime.json` 和 `.../g4-agent-autonomy-20260812-003/agent-autonomy-runtime.json` 均=`PASS`；G4=105 项、contracts=67 项 PASS；四路径 dispatch/action/transition 断言通过，架构门禁在共享契约归位和二次排序动作校验后 PASS。
安全计数：external_llm_requests=0、database/Neo4j/Chroma reads+writes=0、outbox_claims=0、files_deleted=0、database_physical_deletions=0、artifact_overwrites=0。
未解决风险：当前 Agent 自主动作仍是受监督式有限策略，不是允许任意工具或任意循环的开放式自主体；真实 HTTP/Worker 接入、反馈 Agent 的真实在线动作和 G4 真实 LLM 业务探针仍需独立 Gate。
下一步唯一动作：把同一动作协议接入显式 G4 HTTP/反馈边界的只读验证，保持默认 HTTP/Worker 关闭；任何非空 Worker、业务写入或新增外部请求必须单独生成 ChangePlan 并获批。
```

## G4 HTTP 与 G5 反馈边界自主动作记录

```text
交接ID：G4-HTTP-FEEDBACK-AUTONOMY-20260812-003
Gate：G4 HTTP 投影 / G5 feedback-behavior 边界（动作协议只读验证）
状态：CONTRACT_PASS / OFFLINE_BOUNDARY_PASS / DEFAULT_CLOSED
时间：2026-08-12（Asia/Shanghai）
目标：把已冻结的 AgentDecision 协议接入显式 G4 HTTP 投影和 G5 交互边界，确保 HTTP 只输出经过角色白名单校验的局部动作；FeedbackLearningAgent 只根据 receipt 的 replayed/profile-update/state 观察选择下一动作，不直接访问数据库、Outbox 或画像。
新增文件：`backend/app/feedback/application/autonomy.py`；`scripts/verify_g4_http_feedback_autonomy.py`；`tests/g5/test_feedback_autonomy.py`。
修改文件：`backend/app/shared_kernel/contracts/autonomy.py` 增加 JSON decision 解析/校验；`backend/app/recommendation/application/g4_projection.py` 增加 trace action 投影；`backend/app/api/models.py`、`backend/app/api/recommendation.py`、`backend/app/api/feedback.py` 增加 `AgentAction`/`agent_actions`；`contracts/openapi/openapi-v1.json` 与 `docs/api.md` 同步；Makefile 增加 `verify-g4-http-feedback-autonomy`。
角色边界：G4 HTTP `agent_actions` 只包含 step/agent/version/message/action/target/reason/confidence/parameters/evidence_refs；不输出原始 Agent payload、用户文本、凭据或数据库行。G5 FeedbackLearningAgent 的 `PROPOSE_PROFILE_DELTA` 只是提案，目标为 `UserProfileAgent`，不 claim Outbox、不执行画像写入；幂等 replay 固定返回 `RETURN_RESULT`。
验证命令：`make PYTHON=.venv-g1-final-py311/bin/python G4_HTTP_FEEDBACK_AUTONOMY_RUN_ID=g4-http-feedback-autonomy-20260812-003 verify-g4-http-feedback-autonomy`；G4 projection、G5 API/autonomy 单元测试；架构/契约门禁。
验证证据：`artifacts/verification/g4/g4-http-feedback-autonomy-20260812-003/http-feedback-autonomy.json`=`PASS`；G4 HTTP 7 个 Agent action 投影、pending negative=`PROPOSE_PROFILE_DELTA`、replay=`RETURN_RESULT`、no-delta=`RETURN_RESULT` 均通过。
安全计数：external_llm_requests=0、database/Neo4j/Chroma reads+writes=0、outbox_claims=0、files_deleted=0、database_physical_deletions=0、artifact_overwrites=0。
未解决风险：该证据是规则/fake/纯投影边界验证，不等同于真实 G4 HTTP POST、G5 事实追加、非空 Worker 或线上 LLM；默认 HTTP/Worker/DeepSeek 仍关闭。真实业务请求必须独立生成并批准 ChangePlan、plan_id/hash，且先做只读基线和精确回读。
下一步唯一动作：回到 G8 final revalidation 的剩余只读/故障矩阵；任何真实业务 POST、非空 Outbox claim、浏览器写入或 DeepSeek 请求继续单独审批。
```
...
## G8 核心全量复验与 G9 冻结门禁刷新

```text
交接ID：G8-FINAL-25-AND-G9-PREFLIGHT-20260814-001
Gate：G8 A01—A25 final revalidation / G9 formal-input preflight
状态：G8_CORE_25_PASS / G9_PASS_WITH_BLOCKERS
时间：2026-08-14（Asia/Shanghai）
当前绑定提交：82990ba98512fbc7da0bc6ea9e553936bec85be6
最终计划：g8-final-revalidation-plan-20260814-003；plan_hash=1db86eb4a2bd4ccfad2d8f4013edc3f32a53d0fcba726786dc51175f85f60d71。
只读矩阵：g8-readonly-fault-matrix-20260814-003；17 个用例、64 项隔离断言 PASS。
写入证据对账：g8-approved-write-reconcile-20260814-003；复用三批用户精确批准且已执行 PASS 的不可变证据，回读任务/推荐记录、事件与反馈 UUID、曝光阈值、ALREADY_READ 状态、主题负偏好、Outbox DONE 和 profile change log；八个用例全部 PASS。
最终审计：g8-final-revalidation-audit-20260814-001=PASS；A01—A25 为 final_pass=25、final_pending=0、final_fail=0，plan Git 与审计提交匹配。
本轮数据边界：对账仅执行 15 次隔离 MySQL SELECT 并 rollback；database_writes=0、Outbox claims=0、external_llm_requests=0、Neo4j/Chroma writes=0、files_deleted=0、database_physical_deletions=0、artifact_overwrites=0。来源证据所代表的历史批准操作单列为 database_writes=79、Outbox claims=3，不冒充本轮写入。
G9 预检：freeze-20260814-001 与 eval-inputs-20260814-001 均为 PASS_WITH_BLOCKERS。F0 协议已冻结；F1/F2/F3 尚缺满足 evaluation schema 的真实数据与许可 Manifest、盲标注、冻结 Split 和配置 Manifest。synthetic-demo fixture 不具备确认性论文资格；Lib 书目事实可用于本地原型和候选库，但没有用户相关性金标准，不能自动作为正式评测集。
浏览器只读复核：当前默认 Compose 前端可访问，真实展示 backend/MySQL 健康状态、推荐链关闭和 Mock provider；本地演示按钮明确声明不访问 API、不写 MySQL/Neo4j/Chroma。该结果证明默认 fail-closed 正常，不代表显式 DeepSeek 组合根失效；后者已有真实 HTTP 与模型证据。
未解决风险：六场景真实浏览器业务 POST、生产 OIDC/JWKS、性能/恢复报告和全新发布环境复现仍未完成，因此 G8 Gate 保持 IN_PROGRESS；G9 不能在缺少评测输入时伪造开始。
