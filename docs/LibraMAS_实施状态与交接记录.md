...
- `current_subtask`：G4 Agent 自主动作协议与四路径离线运行证据已完成，G8 发布候选只读/构建前置继续保持；A01—A25 已全部达到 DIRECT 离线覆盖；当前计划 `g8-final-revalidation-plan-20260812-004` 与审计 `g8-final-revalidation-audit-20260812-002` 已与提交 `a490652` 匹配。17 项只读路径继续逐项补证，8 项及六个浏览器场景仍需独立 ChangePlan；G7 readiness=`DEGRADED` 仍是公开阻塞项。provider-only DeepSeek fixture 已 PASS；G4 真实 LLM 只读探针未形成 PASS artifact，不能提升为真实业务通过。自主动作验证本身未执行任何数据库/Neo4j/Chroma 读取或写入、Outbox claim、删除或覆盖。
...
- `immediate_risk`：G3/G4/G5 HTTP 仍未进入默认生产配置；G6 图/向量已完成显式组合根的真实只读融合但尚未接入默认 Agent/HTTP，MySQL embedding 状态仍 PENDING，DeepSeek provider-only fixture 已真实调用但 G4 业务探针尚未形成 PASS；Agent 自主动作已在离线 G4 四路径验证，真实业务/HTTP 接入仍待独立 Gate；Prompt Bundle 已冻结但 Explanation/Feedback 的真实 LLM 接线仍待 EvidenceValidator/事务边界评审；G7 推荐工作台与浏览器幂等闭环已完成，feedback/behavior 页面和论文演示冻结仍待完成。任何默认环境仍不得自动开启推荐。Chroma operator 依赖只用于显式导入/校验，不进入默认 backend/worker 镜像。
