...
- `completed_work`：
  ...
  - 已完成 A01—A25 离线覆盖审计：新增 `scripts/verify_g8_acceptance_coverage.py`、`tests/g8/test_acceptance_coverage.py` 和 `make verify-g8-acceptance-coverage`；`artifacts/verification/g8/g8-acceptance-coverage-20260812-001/acceptance-coverage.json` 为 `PASS_WITH_BLOCKERS`，25 项映射全部有效，7 项直接覆盖、14 项相关覆盖、4 项缺少直接测试，25 项最终 G8/G9 复验仍为 `PENDING`。审计只读源码/测试/既有 artifact，数据库、Neo4j、Chroma、外部 LLM、删除和覆盖计数均为 0。
- `open_issues`：
...
- `current_subtask`：G8 发布候选只读/构建前置已通过，扩展报告 `g8-release-preflight-20260812-002` 为 `PASS_WITH_BLOCKERS`；A01—A25 覆盖审计 `g8-acceptance-coverage-20260812-001` 已生成，7 项直接、14 项相关、4 项缺失，映射无陈旧引用。数据库读取/写入、Outbox claim、Neo4j/Chroma 访问和 DeepSeek 请求均为 `0`；下一步优先补齐 A07/A12/A17/A24 的直接离线测试，再规划最终 A01—A25/六场景 E2E 与故障矩阵，仍不能替代真实 G5 ChangePlan。
...
```text
交接ID：G8-A01-A25-COVERAGE-20260812-001
...
实际命令：`make PYTHON=.venv-g1-final-py311/bin/python G8_ACCEPTANCE_COVERAGE_RUN_ID=g8-acceptance-coverage-20260812-001 verify-g8-acceptance-coverage`。
...
证据：`artifacts/verification/g8/g8-acceptance-coverage-20260812-001/acceptance-coverage.json`；运行基线 commit=`1dc3208f77b4f85379d2de4c85c0357cf4a7b2a5`，报告生成前工作区 clean。
