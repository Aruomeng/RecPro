...
本轮已进一步生成并独立校验 `chroma-collection-plan-20260811-001`：状态 `PASS_WITH_WARNINGS`、`can_build=true`，collection=`library_resources__hash_char_ngram_v1`，距离度量=`cosine`，预计记录数=14,983，客户端状态=`NOT_INSTALLED`，写入授权字段仍为必需且未被本计划授予。metadata 必须包含 `external_id`、`vector_id`、`embedding_version`、`index_version`、`namespace_name`、`metadata_version`、`graph_version`、资源类型/分类/难度/可用时间等字段；写策略固定为 `ADD_NEW_COLLECTION_ONLY`、append-only、禁止覆盖、禁止物理删除、禁止活动版本切换。ChangePlan 和验证报告均为本地新建 evidence，`database_reads=0`、`database_writes=0`、`external_store_writes=0`、`actual_delete_count=0`、`files_deleted=0`，未连接或写入 Chroma。

证据目录：`artifacts/verification/chroma-collection-plan/chroma-collection-plan-20260811-001/`、`artifacts/verification/chroma-collection-plan/chroma-collection-verify-20260811-001/`。用户单独确认前不得安装/锁定客户端后执行写入；确认后还需先审查客户端版本、collection 是否已存在、目标记录数和只读回读计数，再讨论 MySQL `embedding_status` 的受控状态投影。
