"""Apply the forward-only G3 recommendation migration with explicit credentials."""
...
DEFAULT_MIGRATION = PROJECT_ROOT / "infra/mysql/migrations/002_g3_recommendation.sql"
...
async def execute(args: argparse.Namespace) -> int:
    ...
    evidence_path = PROJECT_ROOT / "artifacts" / "verification" / "g3" / run_id / "migration.json"
    evidence_path.parent.mkdir(parents=True, exist_ok=False)
    evidence_path.write_text(
        json.dumps(
            {
                "schema_version": "g3-migration-evidence-v1",
                "run_id": run_id,
    ...
    print(f"[PASS] G3 migration {'applied' if args.apply else 'validated'}: {evidence_path}")
    return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[FAIL] G3 migration did not complete: {type(exc).__name__}")
        return 1
