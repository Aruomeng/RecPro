"""Apply the forward-only G4 Agent execution fact migration."""
...
DEFAULT_MIGRATION = PROJECT_ROOT / "infra/mysql/migrations/005_g4_agent_execution.sql"
...
async def execute(args: argparse.Namespace) -> int:
    ...
    if not user or not password:
        raise ValueError("G4 migration credentials are required")
    if args.apply:
    ...
    evidence_path = PROJECT_ROOT / "artifacts" / "verification" / "g4" / run_id / "agent-migration.json"
    evidence_path.parent.mkdir(parents=True, exist_ok=False)
    evidence_path.write_text(
        json.dumps(
            {
                "schema_version": "g4-agent-migration-evidence-v1",
                "run_id": run_id,
    ...
    print(f"[PASS] G4 Agent migration {'applied' if args.apply else 'validated'}: {evidence_path}")
    return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[FAIL] G4 Agent migration did not complete: {type(exc).__name__}")
        return 1
