...
CASE_COVERAGE: Mapping[str, dict[str, Any]] = {
    "A01": {
        ...
        "assessment_reason": "A dedicated replay test compares the profile input hash, event count, and content across two input orderings at the same evaluation boundary.",
        "test_refs": ["tests/g2/test_profile_replay.py::test_profile_snapshot_content_hash_is_replay_stable"],
    ...
    "A02": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "A repeated request_id returns the same task identity and the in-memory persistence fixture records exactly one recommendation record.",
        "test_refs": ["tests/g3/test_recommendation_api.py::test_same_request_replay_keeps_one_recommendation_record"],
    ...
    "A03": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "A repeated direct behavior event_uuid returns replayed status and the fake persistence boundary retains exactly one event fact.",
        "test_refs": ["tests/g5/test_feedback_api.py::test_same_behavior_uuid_replay_keeps_one_event_fact"],
    ...
    "A08": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "A fixed candidate set is scored with and without a TOPIC_NOT_INTERESTED negative signal; the target resource receives a positive penalty and a strictly lower final score.",
        "test_refs": ["tests/g3/test_recommendation_service.py::test_topic_negative_counterfactual_strictly_lowers_target_score"],
    ...
    "A09": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "The exposure boundary test proves max_visible_ratio=0.49 is invalid and the frozen 0.5 boundary is valid.",
        "test_refs": ["tests/g5/test_exposure_boundaries.py::test_exposure_threshold_boundaries_are_closed"],
    ...
    "A10": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "The exposure boundary test proves visible_ms=999 is invalid and the frozen 1000ms boundary is valid.",
        "test_refs": ["tests/g5/test_exposure_boundaries.py::test_exposure_threshold_boundaries_are_closed"],
        "source_refs": ["backend/app/feedback/adapters/mysql.py", "backend/app/feedback/domain/models.py"],
        "tool_refs": ["scripts/verify_g5_http_runtime.py"],
        "artifact_globs": ["artifacts/verification/g5/g5-http-*/http-runtime.json", "artifacts/verification/g7/g7-frontend-browser-mysql-readonly-*/readonly.json"],
    ...
    "A15": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "A parameterized offline matrix removes optional tags, profile signals, behavior events, and the entire catalog while asserting every produced score remains finite and within [0,1].",
        "test_refs": ["tests/g3/test_recommendation_service.py::test_missing_optional_features_keep_all_scores_finite_and_bounded"],
    ...
    "A16": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "A dedicated fingerprint includes fixed config, dataset, seed, evaluation_at, intent, ranked scores, evidence, and warnings; reversing equivalent input order produces the same hash.",
        "test_refs": ["tests/g3/test_recommendation_service.py::test_fixed_snapshot_versions_seed_and_time_have_stable_fingerprint"],
        "source_refs": ["backend/app/recommendation/domain/fingerprint.py", "backend/app/recommendation/application/public.py"],
    ...
    "A18": {
        "offline_assessment": "DIRECT",
        "assessment_reason": "An explicit READING_PATH with one covered difficulty level returns DEGRADED, emits a policy warning/reason code, and does not invent reading stages.",
        "test_refs": ["tests/g4/test_orchestrator.py::test_reading_path_with_one_difficulty_level_is_degraded_without_fake_stages"],
        "source_refs": ["backend/app/recommendation/agents/orchestrator.py", "backend/app/recommendation/agents/rule_agents.py"],
        "tool_refs": ["scripts/verify_g4_orchestrator.py"],
        "artifact_globs": ["artifacts/verification/g4/g4-orchestrator-*/orchestrator.json"],
    },
